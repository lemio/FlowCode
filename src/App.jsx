...
let result = fibonacci(number); //680,380
let result2 = fibonacci(7); //680,490
let result3 = fibonacci(number); //680,260
`;
...
export default function App() {
  ...
  // Ref that FlowCanvas populates so we can call navigateTo(x, y)
  const canvasNavigateRef = useRef(null);
  ...
  useEffect(() => {
    ...
    worker.onmessage = ({ data }) => {
      if (data.id !== evalIdRef.current) return;
      setBindings(data.bindings || {});
      if (data.error) { setEvalError(data.error); addDebug('eval-error', data.error); }
      else             { setEvalError(null); }
    };
    worker.onerror = (e) => {
  ...
  const handleCodeChange = useCallback((value) => {
    ...
    debounceRef.current = setTimeout(() => {
      ...
      if (!result.error) { setParsed(result); setParseError(null); }
      else               { setParseError(result.error); addDebug('parse-error', result.error); }
      triggerEval(value);
    }, 350);
  }, [addDebug, triggerEval]);

  // ── Monaco onMount: cursor position → canvas navigation ─────────────────
  const handleEditorMount = useCallback((editor) => {
    ...
    editor.onDidChangeCursorPosition((e) => {
      ...
      // Look for a //x,y position comment on this line
      const m = line.match(/\/\/\s*(-?\d+(?:\.\d+)?)\s*,\s*(-?\d+(?:\.\d+)?)/);
      if (m && canvasNavigateRef.current) {
        canvasNavigateRef.current.navigateTo(parseFloat(m[1]), parseFloat(m[2]));
      }
    });
  }, []);

  // ── Node drag → update //x,y in source ──────────────────────────────────
  const handleNodePositionChange = useCallback((nodeId, x, y) => {
    // Skip all internal/virtual nodes (they start with '__')
    ...
    const updated = lines.map(line => {
      ...
      if (isFnDecl || isVarDecl) return updatePositionComment(line, x, y);
      return line;
    });
  ...
  const handleGraphConnect = useCallback((connection) => {
    ...
    if (targetHandle !== 'input') {
      addDebug('connect', `Connected ${source} → ${target} (handle: ${targetHandle})`);
      return;
    }

    const newCode = updateVariableInit(currentCode, target, sourceVar);
    if (newCode === currentCode) {
      addDebug('connect', `Could not find variable "${target}" in code.`);
      return;
    }
  ...
  const handleEdgeDelete = useCallback((edge) => {
    ...
    if (edgeType === 'ref' && edge.targetHandle === 'input') {
      const currentCode = editorRef.current ? editorRef.current.getValue() : DEFAULT_CODE;
      const newCode = updateVariableInit(currentCode, edge.target, 'undefined');
      if (editorRef.current) editorRef.current.setValue(newCode);
      const result = parseCode(newCode);
      if (!result.error) setParsed(result);
      triggerEval(newCode);
      addDebug('edge-delete', `Removed reference: ${edge.target} is now undefined`);
    } else {
      addDebug('edge-delete', `Deleted edge type=${edgeType} target=${edge.target}`);
    }
  }, [addDebug, triggerEval]);

  // ── Variable value inline edit ────────────────────────────────────────────
  ...
  // Count only "user-visible" nodes (not internal __ prefixed ones)
  const userNodeCount = parsed.nodes.filter(n => !n.id.startsWith('__')).length;
    ...
    <div style={{ display: 'flex', height: '100vh', background: '#0f172a', overflow: 'hidden', fontFamily: 'monospace' }}>
      ...
      <div style={{ width: '50%', display: 'flex', flexDirection: 'column', borderRight: '2px solid #1e293b' }}>
        ...
        <div style={{
          ...
          <span style={{ marginLeft: 'auto', color: statusColor, fontSize: '11px' }}>{statusText}</span>
          <button
            onClick={() => setDebugOpen(o => !o)}
            style={{
              marginLeft: '8px',
              background: debugLog.some(l => l.type.endsWith('error')) ? '#450a0a' : '#1e293b',
        ...
        <div style={{
          ...
          padding: '8px 16px', maxHeight: '90px', overflowY: 'auto',
        }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
            {Object.entries(bindings).filter(([k]) => !k.startsWith('__')).map(([key, val]) => (
              <div key={key} style={{
                ...
                padding: '2px 8px', fontSize: '11px', display: 'flex', gap: '4px',
              }}>
        ...
        {debugOpen && (
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
              ...
              <button onClick={() => setDebugLog([])} style={{ background: 'none', border: 'none', color: '#475569', cursor: 'pointer', fontSize: '10px' }}>clear</button>
            </div>
            {debugLog.length === 0 && <span style={{ color: '#475569', fontSize: '10px' }}>no events</span>}
            {debugLog.slice().reverse().map((entry, i) => (
              <div key={i} style={{ fontSize: '10px', padding: '1px 0', color: entry.type.endsWith('error') ? '#f87171' : '#94a3b8', fontFamily: 'monospace' }}>
                <span style={{ color: '#475569' }}>{entry.ts} </span>
                <span style={{ color: entry.type === 'parse-error' ? '#fbbf24' : entry.type === 'eval-error' ? '#f87171' : '#34d399', marginRight: '6px' }}>[{entry.type}]</span>
                {entry.msg}
              </div>
            ))}
          </div>
        ...
        {(parseError || evalError) && (
          <div style={{
            ...
            padding: '6px 16px', fontSize: '11px', color: '#fca5a5', fontFamily: 'monospace',
          }}>
      ...
      <div style={{ width: '50%', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{
          ...
          <span style={{ color: '#64748b', fontSize: '11px' }}>
            — {userNodeCount} nodes, {parsed.edges.length} edges
          </span>
          <span style={{ marginLeft: 'auto', color: '#475569', fontSize: '10px' }}>
            drag edge endpoint to reconnect · Backspace to delete
          </span>
        </div>
        ...
        <div style={{ flex: 1, minHeight: 0 }}>
          <FlowCanvas
            ...
            canvasNavigateRef={canvasNavigateRef}
          />
        </div>
      </div>
    </div>
